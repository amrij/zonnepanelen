...
If ($midnight >= $begin) {
	...
	$cols1 = $inverter == 1 ? "v_ac, i_ac, frequency, p_active"
				: "v_ac1, v_ac2, v_ac3, i_ac1, i_ac2, i_ac3, frequency1, frequency2, frequency3, p_active1, p_active2, p_active3";
	$cols2 = $inverter == 1 ? "" : ", p_active1+p_active2+p_active3 p_active";
	// haal de gegevens van de enkel fase inverter op
	$query = sprintf("SELECT datum, MIN(temperature) t_min, MAX(temperature) t_max, temperature t_act,
				MAX(p_active) p_max, MAX(se_day) e_day, mode, v_dc,
				%s%s
			  FROM (
				SELECT temperature, mode, v_dc,
					%s,
			  ...
			  GROUP BY date", $cols1, $cols2, $cols1, $format, $format1, $table, $today, $tomorrow);
	$result = $mysqli->query($query);
	$row = mysqli_fetch_assoc($result);
	$diff['IT']	= $row['datum'];
